...
使用方法:
    python dividend_calculator.py --stock-code 600036 --price 41.2
    python dividend_calculator.py --batch
"""
...
def calculate_payout_ratio_stability(records: List[DividendRecord]) -> Tuple[float, str]:
    ...
    payout_ratios = [r.payout_ratio for r in records]
...
def calculate_base_payout_ratio(records: List[DividendRecord], company_type: CompanyType) -> float:
    ...
    if len(recent_records) < 2:
        return recent_records[0].payout_ratio if recent_records else 0.0

    avg_payout = sum(r.payout_ratio for r in recent_records) / len(recent_records)
    return avg_payout
...
def apply_industry_adjustment(
    ...
    A类：一般不需要纠正，除非有外力（考核目标、管理层表态）
    B类：根据周期情况调整（周期顶部下调，周期底部上调）
    C类：根据资本开支情况调整（有大额投资下调，出售资产上调）
    D类：不推荐
    """
...
def calculate_dividend_yield(
    ...
    # 如果没有指定公司类型，自动判断
    if hasattr(company_data.company_type, 'value'):
        pass  # 已经是有效的CompanyType
    else:
        # 如果传入了字符串，转换为枚举
        if isinstance(company_data.company_type, str):
...
PRESET_COMPANIES = {
    "600036": {
        ...
        "dividend_records": [
            ...
            DividendRecord(2024, 5.6, 1.90, 33.99),
        ]
    },
    "601972": {
        "stock_code": "601972",
        ...
        "industry": "海运",
        "dividend_records": [
            DividendRecord(2023, 2.34, 1.16, 49.74),
            DividendRecord(2024, 2.04, 1.01, 49.4),
        ]
    },
    "000858": {
        "stock_code": "000858",
        ...
        "dividend_records": [
            ...
            DividendRecord(2024, 7.0, 4.9, 70.0),
        ]
    }
}


def batch_calculate() -> List[Dict]:
    """批量计算预设公司的股息率"""
    results = []

    # 五粮液有特殊公告约束
    wuly_factors = AdjustmentFactors(
        min_payout_ratio=70.0,
        min_dividend_per_share=5.15
    )

    for code, data in PRESET_COMPANIES.items():
        company = CompanyData(**data)
        factors = wuly_factors if code == "000858" else AdjustmentFactors()
...
def main():
    parser = argparse.ArgumentParser(description="A股股息率计算器")
    ...
    elif args.stock_code and args.price:
        # 单个股票计算示例
        if args.stock_code in PRESET_COMPANIES:
            data = PRESET_COMPANIES[args.stock_code]
            data["current_price"] = args.price
            company = CompanyData(**data)
            ...
            print(f"❌ 未找到股票 {args.stock_code} 的预设数据")
            print("可用的预设代码:", ", ".join(PRESET_COMPANIES.keys()))
    else:
        parser.print_help()
